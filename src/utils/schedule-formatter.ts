...
interface DaySchedule {
  day: number;
  breakfast: { time: string } | null;
  lunch: { time: string } | null;
  dinner: { time: string } | null;
...
export function formatScheduleToCompact(response: ScheduleResponse): string {
  ...
  response.data.schedule.forEach((daySchedule: DaySchedule) => {
    // Add day header
    lines.push(`D${daySchedule.day}`);
    
    // Add meals
    if (daySchedule.breakfast) lines.push(`B=${daySchedule.breakfast.time}`);
    if (daySchedule.lunch) lines.push(`L=${daySchedule.lunch.time}`);
    if (daySchedule.dinner) lines.push(`D=${daySchedule.dinner.time}`);
    
    // Add attractions
    daySchedule.attractions.forEach((attraction: Attraction) => {
...
export function testFormatter(): string {
