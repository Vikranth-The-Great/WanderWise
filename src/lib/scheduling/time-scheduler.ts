...
} from '../data/distance-calculator';

// Import types from travelai-workflow
/**
 * Raw attraction data interface as parsed from input.
 */
...
/**
 * Structure representing a fully scheduled day.
 */
...
/**
 * Schedules a list of ranked attractions across multiple days.
 * Optimizes for distance, opening hours, and balanced daily load.
 * 
 * @param rankedAttractionNames - List of attraction names in priority order.
 * @param attractionsData - Detailed data for all available attractions.
 * @param tripDuration - Number of days in the trip.
 * @param startDate - Start date of the trip.
 * @param dailyStartTime - Start time for each day (default: 09:00).
 * @param dailyEndTime - End time for each day (default: 18:00).
 * @param maxDayDistanceKm - Maximum travel distance per day.
 * @returns The complete scheduling result including daily itineraries.
 */
...
/**
 * Converts the scheduling result into the frontend-friendly format.
 * Adds intelligent meal scheduling (breakfast, lunch, dinner) around attractions.
 * 
 * @param schedulingResult - The result from scheduleRankedAttractions.
 * @param userPreferences - User preferences for budget, meals, etc.
 * @returns Array of daily plans formatted for UI display.
 */
  ...
  const dayPlans = schedulingResult.schedule.map(daySchedule => {
    ...
    daySchedule.attractions.forEach(attraction => {
      activities.push({
        ...
        cost: getBudgetCost((userPreferences.budget as string) || 'mid-range', 'attraction'),
        type: 'attraction',
    ...
    activities.push({
      time: '08:00',
      title: 'Breakfast',
      location: `Local restaurant near ${userPreferences.destination || userPreferences.destinations?.[0] || 'your destination'}`,
      description: 'Start your day with a delicious local breakfast',
      duration: '1 hour',
      cost: getBudgetCost((userPreferences.budget as string) || 'mid-range', 'meal'),
      type: 'meal'
    });
...
function getBudgetCost(budget: string, type: 'meal' | 'attraction'): string {
  ...
  const multiplier = budgetMultipliers[budget.toLowerCase() as keyof typeof budgetMultipliers] || budgetMultipliers['mid-range'];
