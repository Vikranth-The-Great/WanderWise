 ...
 * @returns The coordinates or null if geocoding failed.
 */
...
export async function reverseGeocode(lat: number, lng: number): Promise<string | null> {
  try {
    const response = await fetch(
      `${BASE_URL}/v1/geocode/reverse?lat=${lat}&lon=${lng}&apiKey=${process.env.NEXT_PUBLIC_GEOAPIFY_API_KEY}`
    );
...
/**
 * Searches for places of specific categories near a geographic location.
 ...
 * @param categories - List of place categories to filter by.
 * @returns Array of PlaceDetails objects.
 */
...
export async function getPlacesAlongRoute(
  ...
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/places?categories=${categories.join(',')}&filter=route:${routeString}&bias=proximity:${routeString}&apiKey=${process.env.NEXT_PUBLIC_GEOAPIFY_API_KEY}&limit=20`
    );
